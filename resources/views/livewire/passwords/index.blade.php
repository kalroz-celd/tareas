    ...
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div class="flex items-center gap-2">
            <button type="button"
                    ...
                    class="rounded-xl bg-indigo-600 px-4 py-2 text-sm font-semibold text-white shadow-sm transition-colors hover:bg-indigo-500">
                Nueva credencial
            </button>
        </div>
    </div>
    ...
    <div class="space-y-4">
        ...
        <div class="rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900 overflow-hidden transition-colors duration-300">
            <div class="hidden md:block">
                <table class="w-full text-sm">
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                        @forelse($entriesByProject as $projectName => $projectEntries)
                            <tr class="bg-slate-50/80 dark:bg-slate-950/40">
                                <td colspan="5" class="px-4 py-3 text-sm font-semibold text-slate-700 dark:text-slate-200">
                                    {{ $projectName }}
                                </td>
                            </tr>
            ...
            <div class="md:hidden divide-y divide-slate-100 dark:divide-slate-800">
                @forelse($entriesByProject as $projectName => $projectEntries)
                    <div class="px-4 py-3 text-sm font-semibold text-slate-700 dark:text-slate-200 bg-slate-50/80 dark:bg-slate-950/40">
                        {{ $projectName }}
                    </div>
                    @foreach($projectEntries as $entry)
